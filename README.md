...
`coyote upgrade [package...]`

Upgrade installed packages to the latest version available in the
index.  If one or more package names are specified, only those packages
are upgraded.  If no package names are specified, all installed packages
are upgraded.

If a specified package is not installed, this command will fail.  If
the installed version is already the latest, no action is taken.  The
upgrade will re-run the `on-install` script if one exists.
